...
namespace Hahadu\ThinkAdmin\View;
use Exception;
use think\Template as TpTemplate;
use Psr\SimpleCache\CacheInterface;

class Template extends TpTemplate
{
    /**
     * 模板变量
     * @var array
    ...
    private function parseTemplateFile(string $template): string
    {
        ...
        $template = $this->parseTemplateFunc($template);
     ...
     */
    private function parseTemplateFunc(string $template){
        if (false !== strpos($template, '@@')){
            $_explode = explode('@@',$template);
            foreach ($_explode as $k => $value){
                if($k==1){
                    $func = $value;
                }
                if($k==0){
